...
**R27 - Reload the engine when it moves under you.** The engine-sync plugin owns
`~/.config/cairnlore/engine-head`: it pulls the clone and writes the stamp on session
start and idle. Never write, create, or repair that file; a missing stamp means no
session has started since install, never an error and never a blocker. Note the clone's
HEAD when you load this file; each turn, if the stamp holds a different SHA, re-read
